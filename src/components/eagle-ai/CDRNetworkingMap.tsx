...
const CDRNetworkingMap = () => {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="cdr-upload" className="block text-sm font-medium mb-2">
                Upload CDR Files (Excel/CSV format with Party A, Party B, Duration, Location columns)
              </Label>
            ...
            {uploadedFiles.length > 0 && (
              <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                  Expected format: Party A | Party B | Duration | Timestamp | Location | Cell Tower
                </p>
              </div>
            )}
          </div>
      ...
      <Tabs defaultValue="enhanced-graph" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="enhanced-graph">Enhanced Network Graph</TabsTrigger>
        ...
        <TabsContent value="detailed-analysis" className="mt-6">
          {analysisResults && (
            <>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className="bg-green-100 dark:bg-green-900/20 p-4 rounded-lg text-center">
                      ...
                      <div className="text-sm text-green-700 dark:text-green-300">Network Strength</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Kingpins (Command Level)
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {analysisResults.kingpins.map((kingpin: any, index: number) => (
                        <div key={index} className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-200">
                          ...
                          <div className="text-sm">
                            ...
                            <div className="text-xs text-gray-500 mt-1">Few contacts, high influence</div>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Middlemen (Coordination)
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      {analysisResults.middlemen.map((middleman: any, index: number) => (
                        <div key={index} className="bg-orange-50 dark:bg-orange-900/20 p-2 rounded border border-orange-200">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Peddlers (Street Level)
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 gap-1 max-h-64 overflow-y-auto">
                      {analysisResults.peddlers.slice(0, 10).map((peddler: any, index: number) => (
                        <div key={index} className="bg-yellow-50 dark:bg-yellow-900/20 p-1 rounded border border-yellow-200">
                          <div className="text-xs">
                            <div className="font-mono text-yellow-800 dark:text-yellow-200">{peddler.phone.slice(-4)}</div>
                    ...
                    <div className="text-xs text-gray-500 mt-2">
                      Showing 10 of {analysisResults.peddlers.length} peddlers
                    </div>
                  </CardContent>
                </Card>
              </div>
